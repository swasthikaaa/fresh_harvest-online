...
// Function to handle item entry and quantity change
function handleItemChange(category) {
  const itemName = document.getElementById(`${category}Name`).value.trim().toLowerCase();
  ...
  const price = prices[itemName] * quantity;
...
// Adding event listeners for all text inputs and quantity inputs
document.querySelectorAll('input[type="text"], input[type="number"]').forEach(element => {
  element.addEventListener('change', (event) => {
      const category = element.id.replace(/Name|Qty/, '');
